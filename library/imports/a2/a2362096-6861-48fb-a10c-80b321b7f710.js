"use strict";
cc._RF.push(module, 'a2362CWaGFI+6EMgLMht/cQ', 'game');
// script/game.js
...
cc.Class({
    ...
    onLoad: function onLoad() {
        ...
        this.horizontal_road_y = -420, this.vertical_road_x = 0;
    ...
    update: function update(dt) {
        ...
        if (!this.isGameOver) this.moveCar(dt);
    },
    moveCar: function moveCar(dt) {
        ...
        if (this.direction == 0) {
            ...
            if (this.gasNode.active == true) this.gasNode.active = false;else this.gameNode.active = true;
        }
        if (this.direction == 1) {
        ...
        if (this.delta_car < 0.1) this.gasNode.active = false;else {
            this.delta_car = 0;
            this.gasNode.active = true;
        }
    },
    changeBackground: function changeBackground(dt) {
    ...
    createPosition: function createPosition() {
        ...
        if (this.direction == 1) {
            ...
            if (random_decimal == 0) this.roadpos_x = this.horizontal_roadNode.x;
            if (random_decimal > 0) this.roadpos_x = Math.ceil(random_decimal * 840 + this.horizontal_roadNode.x - 71);
            if (random_decimal < 0) this.roadpos_x = Math.ceil(random_decimal * 840 + this.horizontal_roadNode.x + 71);
        } else {
            this.roadpos_x = this.vertical_roadNode.x + 420 + 71;
            if (random_decimal == 0) this.roadpos_y = this.vertical_roadNode.y;
            if (random_decimal > 0) this.roadpos_y = Math.ceil(random_decimal * 840 + this.vertical_roadNode.y - 71);
            if (random_decimal < 0) this.roadpos_y = Math.ceil(random_decimal * 840 + this.vertical_roadNode.y + 71);
        }
    ...
    start: function start() {
        this.click_pad.on("touchstart", function () {
            ...
            if (this.direction == 0) this.direction = 1;else if (this.direction == 1) this.direction = 0;
        }, this), this.isloadSoundOn = true;
        if (!CHD.getSoundStatus()) {
        ...
        this.soundBtn.on('touchstart', function () {
            var isSound = CHD.getSoundStatus();
            isSound = !isSound;
    ...
    runTimeAction: function runTimeAction() {
        this.timeNode.scale = 1;
        this.timeNode.stopAllActions();
        var s = cc.scaleTo(3, 0, 1);
